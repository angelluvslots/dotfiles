    ...
    <div className="app">
        ...
        {output.media?.currentSession?.artist && (
          <div className="media">
            <div className="media-buttons button-bg">
              <button
                className={`button no-margin nf ${output.media.currentSession.isPlaying ? "nf-md-pause" : "nf-md-play"}`}
            ...
            <div className="media-artist">
              {limitLength(output.media.currentSession.artist, 24)}
            </div>
            -
            <div className="media-title">
              {limitLength(output.media.currentSession.title, 48)}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="center">
        {output.glazewm && (
          <div
            data-workspace-name={
              output.glazewm.displayedWorkspace.hasFocus
                ? output.glazewm.displayedWorkspace.name
                : ""
            }
            className={`process-name workspace-color ${output.glazewm.displayedWorkspace.hasFocus && "focused"}`}
          >
            {output.glazewm.displayedWorkspace.children[0]
              ? output.glazewm.displayedWorkspace.children.find(
                (child) =>
                  child.id ===
                  output.glazewm.displayedWorkspace.childFocusOrder[0],
              ).processName
              : `[ ${output.glazewm.displayedWorkspace.name} ]`}
          </div>
        )}
